...
def find_score(distri):
    ...
    for n in range(TEN_SCORE,SEVEN_SCORE):
        score +=distri[n]*(11-n)
        '''
        ...
        In PPC shooting, and dictated by the CPCA rulebook, the B29 target is used.
        B29 has X and 10 through 7 rings. Any shot outside of the 7 ring is worth zero.
        In the B29 version of this code, n is in the range of 10..7, and scores are not
        added to total as they are worth zero.
        '''
    # score += distri[ZERO_SCORE]*0; 
    return score
...
def find_distri(score, x, shot_count, show_possible):
    ...
    With each distribution represented as an 8-element list/array,
    ...
    We will iterate from [X,0,0,0,0,0] to
    [X,MAX,MAX,MAX,MAX,0] where MAX is the number of
